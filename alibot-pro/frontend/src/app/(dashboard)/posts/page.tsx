...
function QueueItem({
  ...
  return (
    <div className="flex items-center gap-4 p-4 border-b border-white/[0.04] last:border-0 hover:bg-white/[0.02] transition-colors group">
      {/* Order badge */}
      <div className="w-8 h-8 rounded-lg bg-amber-500/10 border border-amber-500/20 flex items-center justify-center shrink-0">
        <span className="text-[11px] font-bold text-amber-400">{index + 1}</span>
      </div>
      ...
      <div className="flex-1 min-w-0">
        <p className="text-[13px] text-white/80 truncate">{post.product_title}</p>
        <div className="flex items-center gap-3 mt-0.5">
          <span className="text-[11px] text-white/30">#{post.product_id}</span>
          <span className="text-[11px] text-white/30">₪{post.price_ils?.toFixed(2)}</span>
      ...
      <button
        ...
        disabled={removing}
        className="opacity-0 group-hover:opacity-100 flex items-center gap-1.5 px-3 py-1.5 text-[11px] text-red-400/70 hover:text-red-400 hover:bg-red-500/10 rounded-lg transition-all disabled:opacity-40"
        title="הסר מהתור"
      >
    ...
    <div>
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-2 px-3 py-1.5 bg-amber-500/10 border border-amber-500/20 rounded-xl">
            ...
            <span className="text-[13px] font-medium text-amber-400">{queue.length} מוצרים בתור</span>
          </div>
          {queue.length > 0 && (
        ...
        <div className="flex items-center gap-2">
          <button
            onClick={() => load(true)}
            disabled={refreshing}
            className="flex items-center gap-2 px-3 py-1.5 bg-white/5 hover:bg-white/10 disabled:opacity-60 text-white/60 text-[12px] rounded-xl transition-all"
          >
          ...
          <Link
            href="/settings"
            className="flex items-center gap-2 px-3 py-1.5 bg-white/5 hover:bg-white/10 text-white/60 text-[12px] rounded-xl transition-all"
          >
            <Settings2 size={12} />
            הגדרות תזמון
          </Link>
      ...
      {loading ? (
        ...
        <div className="bg-[#0d0f1a] border border-dashed border-white/10 rounded-2xl p-16 text-center">
          ...
          <p className="text-[14px] font-medium text-white/40 mb-1">התור ריק</p>
          <p className="text-[12px] text-white/25">
            לך לעמוד המוצרים ולחץ על "הוסף לתור" כדי להתחיל
          </p>
          <Link
            href="/products"
            className="inline-flex items-center gap-2 mt-4 px-4 py-2 bg-amber-500/10 hover:bg-amber-500/15 border border-amber-500/20 text-amber-400 text-[12px] font-medium rounded-xl transition-all"
          >
            <Package size={13} />
            עבור למוצרים
          </Link>
        ...
        <>
          ...
          <div className="flex items-start gap-3 p-3.5 bg-amber-500/8 border border-amber-500/15 rounded-xl mb-4">
            ...
            <div>
              <p className="text-[12px] text-amber-400/80 font-medium">תור שליחה אוטומטית</p>
          ...
          <div className="bg-[#0d0f1a] border border-white/5 rounded-xl overflow-hidden">
            {/* Column headers */}
            <div className="flex items-center gap-4 px-4 py-2.5 border-b border-white/[0.04] bg-white/[0.02]">
              <div className="w-8 shrink-0" />
              <div className="w-10 shrink-0" />
              <p className="flex-1 text-[11px] font-medium text-white/30">מוצר</p>
              <p className="hidden lg:block w-48 shrink-0 text-[11px] font-medium text-white/30">טקסט</p>
              <div className="w-16" />
            </div>
