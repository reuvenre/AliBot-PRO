...
const SECTIONS: GuideSection[] = [
  ...
  {
    id: 'suppliers', emoji: '📦', title: 'קטלוג ספקים (FLYLINK) והטריילר שבונה אמון', minPlan: 'autopilot',
    body: (
      <>
        <P>טייס מסוג <B>ספקים</B> לא מחפש מילות מפתח — הוא מסובב קטלוג ספק שקישרתם (Yupoo/FLYLINK) ומפרסם ממנו פריט אחרי פריט. במסך <B>ספקים</B> מקשרים קטלוג, והמערכת מושכת את כל התמונות האמיתיות של כל פריט.</P>
        <H>למה התמונות הן העסק כאן</H>
        <P>בקטלוג ספק אין שם מוצר — יש <B>קוד</B> (למשל MM-2642001DP). לכן ה-AI כותב את הפוסט <B>מהתמונות עצמן</B> (ראייה ממוחשבת על 3 צילומים אמיתיים), ולא מהכותרת. זו הסיבה שהטקסט מתאר חומר, תפרים וגוון — הוא באמת מסתכל על הפריט.</P>
        <H>הטריילר: חוזה ציפיות</H>
        <P>מתחת לכל פוסט ספקים נוספת אוטומטית חתימה קבועה. היא נבנית <B>בקוד ולא על ידי ה-AI</B> — בכוונה: ניסוח שלא יכול להשתנות, להסחף או להבטיח משהו שלא נעמוד מאחוריו.</P>
        <div className="bg-white/[0.03] border border-edge rounded-xl p-3.5 my-3 text-sm text-white/70 leading-relaxed whitespace-pre-line">
          {'✅ מה שבתמונה = מה שבקופסה — אותו פריט, אותו צבע, אותם פרטים\n'
            + '📸 אלה צילומי המחסן של הפריט הזה, לא תמונות קטלוג\n'
            + '🏷️ המוצר הינו רפליקה של המקור באיכות גבוהה\n'
            + '📦 מספר מעקב ישירות למייל · משלוח מהיר\n'
            + '💬 שאלה לפני ההזמנה? שלחו הודעה לפרטי ונענה בשמחה'}
        </div>
        <P><B>השורה השנייה היא הנכס.</B> &quot;מה שרואים זה מה שמגיע&quot; זה מה שכל דרופשיפר כותב, ולכן זה לא שוקל כלום. &quot;אלה צילומי המחסן של הפריט הזה&quot; הוא לא שגרתי, הוא נכון, והוא הסיבה שהשורה הראשונה היא הבטחה ולא סיסמה.</P>
        <H>שורת הרפליקה — טלגרם ווואטסאפ בלבד</H>
        <P>השורה 🏷️ מופיעה <B>רק בערוצים שלכם</B>. מטא ופינטרסט אוסרות על מוצרי רפליקה מפורשות, והמילה בעברית היא ההתאמה הקלה ביותר לאכיפה האוטומטית שלהן — כלומר השורה שנועדה להגן על הקונה הייתה זו שמסכנת את העמוד. בפייסבוק, אינסטגרם ופינטרסט הפוסט יוצא זהה בדיוק, בלי השורה הזו.</P>
        <H>מה במכוון לא כתוב שם</H>
        <ul className="list-disc pr-5 space-y-1.5">
          <LI><B>&quot;לא מהיצרן&quot;</B> — המחסן עשוי להיות של היצרן. לא נעמוד מאחורי זה.</LI>
          <LI><B>מספר תמונות</B> (&quot;6 תמונות אמיתיות&quot;) — פוסט יכול לצאת עם תמונה אחת, ואז זה שקר.</LI>
          <LI><B>זמן משלוח בימים</B> — אין שליטה על הלוגיסטיקה.</LI>
        </ul>
        <P>הכלל: כל שורה שם חייבת לשרוד קונה שמצטט אותה בחזרה. ניסוח כן ממיר <B>טוב יותר</B> — קונה שציפה לרפליקה טובה וקיבל אחת ממליץ על הקבוצה; קונה שהובטח לו העולם מבקש החזר.</P>
        <P>הטריילר מתווסף ברגע השליחה, לא בזמן הכתיבה — אז גם פוסטים שכבר בתור, ופרסום חוזר של פוסט ישן, מקבלים את הנוסח העדכני.</P>
      </>
    ),
  },
  {
