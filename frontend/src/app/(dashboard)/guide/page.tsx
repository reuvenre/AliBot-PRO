  ...
  },
  {
    ...
    body: (
      <>
        ...
        <H>בדיקת מילות מותג</H>
        <P>בכפתור <B>&quot;בדוק מילות מותג&quot;</B> במסך הטייסים המערכת סורקת את כל מילות המפתח בכל הטייסים ומסמנת שתי קבוצות: <B>סיכון גבוה</B> — שמות מותג שנתבעים הרבה על זיופים (נייקי, גוצ&apos;י, רולקס, דיסני, לגו…) וניסוחים כמו &quot;replica&quot; ו-&quot;1:1&quot;; ו<B>לבדיקה</B> — מותגים שיש להם חנות רשמית ב-AliExpress (שיאומי, קסיו, בוש) וניסוחים כמו &quot;official/authentic&quot;. לכל ממצא מוצעת חלופה גנרית. שום דבר לא נמחק אוטומטית — ההחלטה שלך, כי רק אתה יודע מה אתה מוכר במכוון. חשוב: פרסום מוצר מזויף בפייסבוק או בפינטרסט מסכן את <B>העמוד</B>, לא רק את הפוסט.</P>
        <H>פילטרים</H>
  ...
  },
  {
    ...
    body: (
      <>
        ...
        <P><B>רצף השקה אוטומטי:</B> שמירת קופונים עם תאריכי תוקף בונה עד 4 פוסטים מתוזמנים לקבוצות האלי אקספרס: <B>טיזר</B> בערב שלפני (&quot;תתחילו למלא עגלה&quot;), <B>השקה</B> בבוקר הכניסה לתוקף עם סולם הקופונים המלא, <B>עוגן</B> באמצע החלון — מוצר אמיתי שמחירו יושב בדיוק מעל סף, ו<B>דחיפות</B> 12 שעות לפני הפקיעה. כל המספרים והקודים נבנים בקוד מהנתונים (לא על ידי ה-AI — כדי שמספר שגוי לא יגיע לקבוצה); ה-AI תורם את שורת הפתיחה. הפוסטים מופיעים ב&quot;פוסטים מיוחדים&quot; וניתנים לעריכה/מחיקה, ושמירה חוזרת של קופונים בונה את הרצף מחדש.</P>
      </>
    ),
  },
  {
  ...
  },
  {
    ...
    body: (
      <>
        ...
        <P><B>איך הפוסט נראה:</B> וואטסאפ לא תומך בקישור מוסתר מאחורי טקסט כמו בטלגרם — הכתובת תמיד מוצגת. לכן המערכת ממירה את הפוסט לעיצוב של וואטסאפ: כותרת ומחיר <B>מודגשים</B>, מחיר קודם עם קו חוצה, ושורת קריאה לפעולה מודגשת ומעליה הקישור לבדו בשורה נפרדת — כך הוא נלחץ בלחיצה אחת ולא נשבר.</P>
        <P><B>ערוצים (Channels) — לא נתמך:</B> נבדק מול Green API באוגוסט 2026 — ה-API שלהם מזהה ערוץ ברשימת הצ&apos;אטים, אבל <B>דוחה שליחה אליו</B>: הוא מקבל רק מספר אישי, chat_id@lid או קבוצה (@g.us). בהגדרות יש כפתור &quot;בדוק תמיכה בערוצים&quot; ולידו &quot;שלח בדיקה&quot; — אם Green API יוסיפו תמיכה בעתיד, הבדיקה תגלה את זה. עד אז: פרסום אוטומטי לוואטסאפ = קבוצות בלבד.</P>
        <P><B>הגנה מחסימות:</B> וואטסאפ חוסמת מספרים לפי <B>התנהגות</B>, לא לפי תוכן. כל הודעה יוצאת בהשהיה אקראית קצרה ובפער מינימלי מההודעה הקודמת, כדי שהפרסום לא ייראה כמו בוט ששולח בדיוק בדקה עגולה. מומלץ בנוסף: לא יותר מ-3–5 פוסטים ביום לקבוצה, מספר ותיק (לא סים חדש), ולוודא שהחברים בקבוצה שמרו את המספר באנשי הקשר.</P>
      </>
    ),
  },
  {
