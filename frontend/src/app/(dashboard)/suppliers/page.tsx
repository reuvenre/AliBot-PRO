...
import {
  ...
  Maximize2, ChevronLeft, ChevronRight, Check, Bot, BotOff, EyeOff, Eye,
} from 'lucide-react';
...
function SupplierRow({ product, catalogName, onCompose, onEdit, reload }: {
  ...
  // Off the public shelf and nothing else — the product stays in the campaign rotation
  // and keeps the FLYLINK link, which was generated by hand and cannot be regenerated.
  const handleStoreToggle = async () => {
    try { await suppliersApi.updateProduct(product.id, { store_hidden: !product.store_hidden } as any); reload(); }
    catch (e: any) { alert(e?.response?.data?.message || 'עדכון נכשל'); }
  };
    ...
    <tr className="border-b border-edge hover:bg-white/[0.02] transition-colors">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {product.store_hidden && <span className="px-1.5 py-0.5 bg-amber-500/15 border border-amber-500/25 text-[9px] text-amber-400 rounded-md font-medium" title="המוצר לא מוצג בחנות הפומבית">מוסתר באתר</span>}
            </div>
          </div>
        </div>
      </td>
      ...
      <td className="px-4 py-3">
        <div className="flex items-center gap-0.5">
          ...
          <ActionBtn icon={product.store_hidden ? EyeOff : Eye}
            label={product.store_hidden ? 'המוצר מוסתר באתר — לחץ להחזרה לחנות' : 'הסר מהאתר (נשאר בקטלוג ובקמפיינים)'}
            onClick={handleStoreToggle} color={product.store_hidden ? 'red' : 'blue'} />
...
function SupplierEditModal({ product, onClose, onSaved }: { product: SupplierProduct; onClose: () => void; onSaved: () => void }) {
  const [form, setForm] = useState({
    ...
    store_brand: product.store_brand || '', store_hidden: !!product.store_hidden,
  });
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState('');
  ...
  const save = async () => {
    ...
    try {
      await suppliersApi.updateProduct(product.id, {
        ...
        store_hidden: form.store_hidden,
      } as any);
      onSaved();
    } catch (e: any) { setError(e?.response?.data?.message || 'שמירה נכשלה'); }
    finally { setSaving(false); }
        ...
        <div className="space-y-3.5">
          ...
          <div className="pt-3 mt-1 border-t border-edge space-y-3.5">
            ...
            <label className="flex items-start gap-2.5 cursor-pointer">
              <input type="checkbox" checked={form.store_hidden}
                onChange={(e) => setForm((f) => ({ ...f, store_hidden: e.target.checked }))}
                className="mt-0.5 w-4 h-4 accent-blue-500" />
              <span className="text-xs text-white/70">
                אל תציג מוצר זה באתר
                <span className="block text-2xs text-white/35 mt-0.5">המוצר נשאר בקטלוג, בקמפיינים ובקישור השותפים — רק יורד מהמדף הפומבי.</span>
              </span>
            </label>
          </div>
          {error && <p className="text-xs text-red-400">{error}</p>}
